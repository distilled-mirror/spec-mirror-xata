...
![snapshots diagram](https://raw.githubusercontent.com/xataio/pgstream/main/docs/img/pgstream_snapshot_diagram.svg)
...
* Data: it relies on transaction snapshot ids to obtain a stable view of the database tables, and parallelises the read of all the rows by dividing them into ranges using the `ctid`.

![snapshots sequence](https://raw.githubusercontent.com/xataio/pgstream/main/docs/img/pgstream_snapshot_sequence.svg)
...
To keep snapshot load off a primary that cannot afford it, the snapshot can be taken from a physical read replica instead. See [Running pgstream from a read replica](/docs/opensource/pgstream/docs/replicas).
