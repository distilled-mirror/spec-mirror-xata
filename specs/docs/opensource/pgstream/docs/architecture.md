...
![pgstream architecture v2](https://raw.githubusercontent.com/xataio/pgstream/main/docs/img/pgstream_diagram_v2.svg)

![pgstream architecture with kafka v2](https://raw.githubusercontent.com/xataio/pgstream/main/docs/img/pgstream_diagram_v2_kafka.svg)
...
* **Filter**: allows to filter out WAL events for certain schemas/tables. It can be configured by providing either an include or exclude table list. WAL events for the tables in the include list will be processed, while those for the tables in the exclude list or not present in the include list will be skipped. The format for the lists is similar to the snapshot tables, tables are expected to be schema qualified, and if not, the `public` schema will be assumed. Wildcards are supported, but not regex. Example of table list: `["test_table", "public.test_table", "test_schema.test_table", "test_schema.*", "*.test", "*.*"]`. By default, the filter will include the `pgstream.schema_log` table in the include table list, since pgstream relies on it to replicate DDL changes. It can be disabled by adding it to the exclude list.
  ...
  A third list, `schema_only_tables`, can be combined with either the include or the exclude list (or used on its own): DDL (schema change) events for these tables are processed, while their data (DML) events are skipped. This keeps the table schema in sync on the target without replicating its data. The precedence rules are: the exclude list wins over everything; a table listed explicitly (no wildcards) in the include list gets full treatment even when it matches a schema-only wildcard; otherwise a schema-only match keeps DDL and drops DML. A DDL statement referencing multiple tables is replayed as a whole and is skipped if any of the referenced tables is filtered out for DDL (schema-only tables count as allowed).

* **Transformer**: it modifies the column values in insert/update events according to the rules defined in the configured yaml file. It can be used for anonymising data from the source Postgres database. An example of the rules definition file can be found in the repo under `transformer_rules.yaml`. The rules have per column granularity, and certain transformers from opensource sources, such as greenmask or neosync, are supported. More details can be found in the [transformers documentation](/docs/opensource/pgstream/docs/transformers).
...
![v1 schema replication](https://raw.githubusercontent.com/xataio/pgstream/main/docs/img/pgstream_v1_schema_replication.png)
