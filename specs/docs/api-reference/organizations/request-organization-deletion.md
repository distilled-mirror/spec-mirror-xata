...
````yaml https://api.xata.tech/openapi.json post /organizations/{organizationID}/deletion-request
openapi: 3.0.0
info:
  title: Xata API
...
paths:
  /organizations/{organizationID}/deletion-request:
    ...
    post:
      ...
      description: >
        Flags the organization for deletion. The request is rejected if the
        organization still has active projects.

        The subscription is cancelled immediately and any outstanding invoices
        are collected automatically after a grace period that prevents missing
        delayed usage reports.

        Deletion completes asynchronously once all invoices are issued and
        settled.
      operationId: requestOrganizationDeletion
      responses:
        '202':
          description: >-
            Deletion requested — the organization will be removed once all
